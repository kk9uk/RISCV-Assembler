...
int inst_to_binary(
    ...
    if (is_opcode(opcode) == ADDI) {
        ...
        binary = (0x20 << 25) + (0x05 << 12) + (0x04 << 2) + 0x03;
        binary += (reg_to_num(arg1, line_no) << 7);
        binary += (reg_to_num(arg2, line_no) << 15);
        binary += (lower5bit(arg3, line_no) << 20);
    ...
    // Unconditional Jumps
        ...
        binary = (0x19 << 2) + 0x03;
        binary += (reg_to_num(arg1, line_no) << 7);
        struct_regs_indirect_addr* parsed = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(parsed->reg, line_no) << 15);
        binary += (parsed->imm << 20);
    } else if (is_opcode(opcode) == JAL) {
        binary = (0x1B << 2) + 0x03;
        binary += (reg_to_num(arg1, line_no) << 7);
        int offset = handle_label_or_imm(line_no, arg2, label_table, number_of_labels) - addr;
    ...
    // Load and Store Instructions
        ...
        struct_regs_indirect_addr* parsed = parse_regs_indirect_addr(arg2, line_no);
        binary += (reg_to_num(parsed->reg, line_no) << 15);
        binary += ((parsed->imm & 0x1F) << 7);
        binary += ((parsed->imm & 0xFE0) << 20);
